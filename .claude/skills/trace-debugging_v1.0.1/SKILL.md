...
call, not per CLI lifecycle event - there's no `turn_id`/`sequence_id`
granularity from this source (both always `0`), and `parent_session_id` is
always blank. `session_id` comes from the
