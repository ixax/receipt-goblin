...
INGEST_API_URL = os.environ.get("CLAUDE_TRACKING_API_URL", "http://localhost:8000")
REQUEST_TIMEOUT = float(os.environ.get("CLAUDE_TRACKING_TIMEOUT", "3"))
DEBUG = os.environ.get("CLAUDE_TRACKING_DEBUG") == "1"
...
def parse_frontmatter(filepath):
    ...
    supported; values are returned as plain strings.
    """
    path = Path(filepath)
    try:
...
def _state_dir(namespace=".claude"):
    # Claude Code and Codex CLI hooks both import this module directly (no
    # copy-paste of the state-file logic) but must not share one state
    # directory - session_id spaces differ per product, and mixing them
    # makes the on-disk state harder to inspect. Every public function
    # below takes this same `namespace` param (default ".claude", so
    # Claude Code's call sites don't need to pass it); Codex's hook
    # scripts pass namespace=".codex" explicitly on every call so their
    # turn/sequence/latency state lands in .codex/.tracking-state instead.
    d = _project_dir() / namespace / ".tracking-state"
    d.mkdir(parents=True, exist_ok=True)
    return d


def _session_state_file(session_id, namespace=".claude"):
    safe = session_id.replace("/", "_") or "unknown-session"
    return _state_dir(namespace) / f"{safe}.json"


def _load_state(session_id, namespace=".claude"):
    f = _session_state_file(session_id, namespace)
    try:
...
def _save_state(session_id, state, namespace=".claude"):
    f = _session_state_file(session_id, namespace)
    try:
...
def next_ids(session_id, new_turn=False, namespace=".claude"):
    ...
    """
    state = _load_state(session_id, namespace)
    if new_turn:
        state["turn_id"] = state.get("turn_id", 0) + 1
        state["sequence_id"] = 0
    state["sequence_id"] = state.get("sequence_id", 0) + 1
    _save_state(session_id, state, namespace)
    return state["turn_id"], state["sequence_id"]


def mark_tool_start(session_id, tool_key, namespace=".claude"):
...
def pop_tool_latency_ms(session_id, tool_key, namespace=".claude"):
...
def mark_permission_request(session_id, tool_key, namespace=".claude"):
...
def pop_permission_wait_ms(session_id, tool_key, namespace=".claude"):
...
def get_state_value(session_id, key, default=None, namespace=".claude"):
    return _load_state(session_id, namespace).get(key, default)


def set_state_value(session_id, key, value, namespace=".claude"):
    state = _load_state(session_id, namespace)
    state[key] = value
    _save_state(session_id, state, namespace)
