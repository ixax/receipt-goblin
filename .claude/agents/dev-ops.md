...
Read `Skill(service-lifecycle)` before every restart/rebuild/recreate decision, before running a backup/restore target, and before editing Makefile/docker-compose.yml - it has the decision tree, the target comparison table, exact commands, and the verification checklist.
...
Run this per `Skill(service-lifecycle)`'s "One-off recreate with env overrides" - a deliberate exception to "never raw `docker compose`"; don't "correct" it back.
`--no-deps` here is safe only because `loadtest-runner`'s Phase 2 health check already confirmed `clickhouse`/`redis` healthy.

After recreating: verify the three report healthy (per the skill's "Verify before reporting done"), then report a confirmation `loadtest-runner` can trust without re-checking them.
