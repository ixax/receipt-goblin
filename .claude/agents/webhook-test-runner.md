...
That runs a separate pytest invocation per service (`services/worker/tests`, `services/reparse/tests`, `services/loadtest/tests`, `services/_common/tests`) against `services/webhook/pytest.ini` - the pure (no-live-ClickHouse) functions in `services/_common/src/ingest_parsing.py` and the ClickHouse-I/O functions in `services/_common/src/ingest_db.py`, using real captured payloads under `services/_common/tests/captures/` (see `services/_common/tests/conftest.py`).
