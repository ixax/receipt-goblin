...
`mcp__dev__query` (e.g. `SELECT count() FROM agent_usage`) rather
...
5. If the rewrite changes what the query *returns* (not just how it runs),
   verify that separately via `mcp__dev__query` on both versions and
   ...
   has no `mcp__dev__query`, so this check is yours, not delegated.)

**C. A one-off query that isn't (yet) a dashboard panel** (e.g. a candidate
rewrite you're drafting before proposing it): use
`mcp__dev__profile_query` yourself directly - no need to route a
...
would help - but you never create one yourself. `mcp__dev__query`
