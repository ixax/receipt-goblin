...
You execute one of two mechanical jobs against `services/grafana/scripts/query_perf.py` (read that file's own docstring first if anything below is unclear - it's the source of truth, this file is just the operating procedure).
You never improvise the SQL-substitution logic yourself.
