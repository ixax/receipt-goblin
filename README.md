...
| ------------------------------------------------------------------ | ---------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `LITELLM_MASTER_KEY`         | required, no default                 | litellm - admin credential for `/ui` and `/key/generate`; real Anthropic/OpenAI keys and per-person virtual keys are managed through the UI instead, see "LiteLLM" below. Also used by webhook to call LiteLLM's `/key/info` when verifying `hooks/report_git_branch.py`'s virtual key.                                                                                                                              |
...
| `OLLAMA_HOST`                | unset                                | litellm - LAN address of the machine running Ollama (a separate host, not a `docker-compose.yml` service). Optional, and unset by default (not just defaulted to `localhost`) - `local_reasoning`/`local_embeddings` only get defined at all once it's set, see "Local Ollama models" under "LiteLLM" below                                                                                                          |
| `OLLAMA_PORT`                | `11434`                              | litellm - Ollama's port on that same host                                                                                                                                                                                                                                                                                                                                                                            |
| `OLLAMA_REASONING_MODEL`     | `ollama/gemma3:4b`                   | litellm - real Ollama tag behind the stable `local_reasoning` `model_name`, see "Local Ollama models" below                                                                                                                                                                                                                                                                                                          |
| `OLLAMA_EMBEDDINGS_MODEL`    | `ollama/embeddinggemma:300m`         | litellm - real Ollama tag behind the stable `local_embeddings` `model_name`, see "Local Ollama models" below                                                                                                                                                                                                                                                                                                         |
| `AGENT_CLI_TRACKING_API_URL` | required, no default                 | `hooks/report_git_branch.py` - not a `docker-compose.yml`/`.env` variable, exported into your shell instead (see `make env` and "Git branch/repo" below). No fallback: if unset, the hook crashes (`KeyError`, non-zero exit) instead of guessing a URL.                                                                                                                                                              |
| `LITELLM_VIRTUAL_KEY`        | required, no default                 | `hooks/report_git_branch.py` - personal virtual key (see `make env`), sent as `Authorization: Bearer` on every git-branch report; webhook verifies it against LiteLLM's own `/key/info` before accepting the report. No fallback: hook crashes if unset, same as `AGENT_CLI_TRACKING_API_URL`.                                                                                                                       |
...
`local_reasoning` (→ `ollama/gemma3:4b`) and `local_embeddings` (→ `ollama/embeddinggemma:300m`, `model_info.mode: embedding`) route to Ollama running on a separate machine on the LAN (`OLLAMA_HOST`/`OLLAMA_PORT`, see "Configuration" above), not a `docker-compose.yml` service - unlike the Anthropic entries above, there's no OAuth passthrough involved, LiteLLM talks to that Ollama server directly.
These two `model_name` entries live in their own `services/litellm/config.ollama.yaml`, not `services/litellm/config.yaml` - `services/litellm/docker-entrypoint.sh` merges it in via LiteLLM's own `include:` directive ([config file docs](https://docs.litellm.ai/docs/proxy/config_management)), and only when `OLLAMA_HOST` is actually set in the container's environment. `OLLAMA_HOST`/`OLLAMA_PORT` are optional - `docker-compose.yml` always starts either way; leave `OLLAMA_HOST` unset and `local_reasoning`/`local_embeddings` simply don't exist as models at all (not "exist but fail"), instead of every request needing to discover that no Ollama is configured.
The real Ollama tag behind each of these two `model_name`s comes from `OLLAMA_REASONING_MODEL`/`OLLAMA_EMBEDDINGS_MODEL` rather than being hardcoded in `services/litellm/config.yaml` - swapping the tag (e.g. a bigger/smaller gemma3, or a different embedding model entirely) only needs a `.env` change and `docker compose up -d litellm`, the public `model_name` callers use never changes.
The tag set in `OLLAMA_REASONING_MODEL`/`OLLAMA_EMBEDDINGS_MODEL` must already be pulled on the Ollama host (`ollama pull gemma3:4b`, `ollama pull embeddinggemma:300m`) - LiteLLM doesn't pull models itself.
Ollama must also be listening on `0.0.0.0`, not just `localhost`, on its own host (`OLLAMA_HOST=0.0.0.0` in Ollama's own environment - not to be confused with this project's `OLLAMA_HOST`, which is the address *litellm* dials, the reverse direction) or the `litellm` container can't reach it across the LAN.
