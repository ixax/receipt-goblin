...
6. Reads go the other way: `webhook`/`webhook-worker` are write-only, so a CLI session reads data back out via one of two MCP servers registered in `.mcp.json` - `mcp-dev` on `:8001` (dev-only, arbitrary read-only SQL) and `mcp-stats` on `:8002` (prod, e.g. `/whatsup` in Claude Code, requires a LiteLLM virtual key).
...
This starts the core stack only.
...
Always go through `make build`/`make up` rather than calling `docker compose build`/`docker compose up` directly - the `Makefile` resolves each service's image tag from `VERSION.yml` first (`scripts/resolve_image_version.py`) and exports it before invoking `docker compose`; a raw `docker compose build`/`up` skips that resolution and leaves you with an image tagged out of step with `VERSION.yml`.
...
Calls the `mcp-stats` MCP server (`mcp__stats__whatsup`, see `.mcp.json`) and reports tokens/cost spent in the last 24h, plus the top spenders - no need to open Grafana for a quick check.
...
| `/whatsup` fails or times out                                                                                                                                                                                                                        | Confirm `mcp-stats` is `healthy`/running (`docker compose ps`), reachable at `http://localhost:8002/mcp`, and that your `LITELLM_VIRTUAL_KEY` is set/not blocked/expired (401 means the auth check failed - see "MCP servers" under "Reference" below); check `docker compose logs mcp-stats`. Claude Code only picks up `.mcp.json` changes on the next session start.                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| **ClickHouse**                                   |                                                                  |                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| `CLICKHOUSE_DATABASE`                            | required                                                         | clickhouse, clickhouse-migrate, webhook, worker, webhook-reparse, mcp-dev (dev-only), mcp-stats, grafana, backup                                                                                                                                                                                                                                                                                                                                                                       |
...
| `CLICKHOUSE_MCP_STATS_USER`/`_PASSWORD`          | required                                                         | mcp-stats - `mcp_stats` role: read-only, scoped to `agent_usage` only (the one table `whatsup` reads)                                                                                                                                                                                                                                                                                                                                                                           |
...
`webhook`/`worker`/`webhook-reparse` connect as `ingest`, `grafana` as `grafana`, `mcp-dev` (dev-only) as `mcp`, `mcp-stats` as `mcp_stats`, `backup` as `backup`, `loadtest` as `loadtest` (on its own separate `CLICKHOUSE_LOADTEST_DATABASE`).
...
| `ingest_raw`         | Full, untouched original `StandardLoggingPayload` per call (`messages` included), write-once at ingest time, compressed hard (`ZSTD(19)`) since it's read only by `webhook/src/reparse.py`, never by a live dashboard query. This is what makes reparsing possible.                                                                                                                   |
...
- `query(sql: str, max_rows: int = 200)` - arbitrary SQL from the model, for the `clickhouse-analyst` subagent (see `.claude/agents/clickhouse-analyst.md`) and ad hoc lookups.
...
- `whatsup(hours: int = 24)` - three fixed queries (total tokens, total cost from `agent_usage.cost`, top 5 spenders). Read-only by construction - never runs arbitrary SQL from the model. Its ClickHouse identity (`mcp_stats` role) is scoped to `agent_usage` only, unlike `mcp-dev`'s broad `mcp` role.
...
`make setup-client` (see "Getting started" above) prints `export` statements with a `<virtual key>` placeholder for `ANTHROPIC_BASE_URL`/`ANTHROPIC_CUSTOM_HEADERS`/etc. (`LITELLM_PORT` in `.env` control the URL if you changed it from the default).
...
So the virtual key has to be the actual value, not a reference to whatever's already exported in your shell; that's also why `make setup-client` substitutes it in directly rather than printing something like `"$LITELLM_VIRTUAL_KEY"`.
...
All six Langfuse services carry `profiles: [langfuse]` in `docker-compose.yml`, so a plain `docker compose up -d` **never starts them** - only the core stack (`clickhouse`, `redis`, `webhook`, `webhook-worker`, `grafana`, `litellm`, `litellm-db`, `mcp-stats`) comes up by default, plus `mcp-dev` too but only in `ENVIRONMENT=development` (it's defined solely in `docker-compose.dev.yml` and absent in production - see "Dev vs prod" above).
...
| `cadvisor`       | Stock `gcr.io/cadvisor/cadvisor` image. Per-container cgroup metrics (CPU/memory/network per `receipt-goblin-*` container), read-only host mounts (`docker.sock`, `/sys`, `/var/lib/docker`, ...) for that, no config of its own. Broken under the containerd-snapshotter storage driver (current Docker Desktop default, also selectable under Colima) - see the "Containers" tab row under "Troubleshooting" above; kept anyway since it doesn't affect the other observability services.              |
