...
It needs `AGENT_CLI_TRACKING_API_URL` set in your shell to know where to POST to - `make env` prints an `export` line for it (see "Issue yourself a personal key" above); there's no fallback, so an unset/missing variable crashes the hook (`KeyError`, non-zero exit) rather than guessing a URL.
