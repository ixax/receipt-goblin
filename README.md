...
`make test` runs `services/webhook/tests` - a pytest suite for every function in `services/webhook/src/clickhouse_ingest.py` that doesn't touch ClickHouse (row/id/text parsing, not the `client.insert`/`client.query` calls).
It needs no live ClickHouse or docker-compose stack: `services/webhook/tests/conftest.py` stubs the required `CLICKHOUSE_*` env vars before import, and every test loads a real captured payload from `services/webhook/tests/captures/*.json` (copies of actual `services/webhook/captures/<session_id>/*.json` files) rather than hand-built fixtures.
