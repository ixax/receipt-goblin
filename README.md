...
Recommended: run `make init` first - it installs `uv` itself if it isn't already on `PATH`, then opens by asking whether this is a development or production setup (see "Dev vs prod" below) before anything else runs.
It then interactively asks for the database name, a bootstrap superuser, and a username/password for each of the five ClickHouse roles (generating any password you leave blank), copies `.env.example` to `.env` if it doesn't exist yet, writes all of that in, then brings up just `clickhouse` long enough to create those users before stopping it again.
It also applies all ClickHouse schema migrations automatically.
Then it brings up `litellm` long enough to create a default Team and a personal virtual key for you via LiteLLM's own Admin API, writes `LITELLM_TEAM_ID`/`LITELLM_VIRTUAL_KEY` into `.env`, then stops `litellm` again - no manual `/ui` step needed for your first key, see "Issue yourself a personal key" below.
Finally it runs `make setup-client` for you, printing ready-to-paste shell-export lines and Codex/Claude Code config blocks with that new key already filled in.
It then prints a reminder to run `make up` next (and `make observability-up` if you want that opt-in profile too).
Continue to "Start the stack" below.
...
`make init` asks for this up front, before anything else - see "Environment variables" above.
...
`make init` already created a personal key for you (a default Team plus a virtual key with access to every model, written into `.env` as `LITELLM_TEAM_ID`/`LITELLM_VIRTUAL_KEY`) and printed the ready-to-paste `make setup-client` output for it at the end of that run.
There's nothing to do in the UI for your first key - just route a client through the proxy:

1. Check the model you need is actually listed: **Models + Endpoints** (http://localhost:4000/ui/models-and-endpoints), logging in with `admin` / your `LITELLM_MASTER_KEY`. If it isn't, don't add it there - go set it up under "Remote model sources" / "Model name mapping" under "LiteLLM" below instead, then come back.
2. Paste the shell-export lines `make init` printed into `~/.zshrc`/`~/.bashrc`, or merge the printed Codex/Claude Code config blocks into `~/.codex/config.toml`/`~/.claude/settings.json` instead (no shell rc edit needed) - see "Routing Claude Code through it" / "Routing Codex CLI through it" / "Configuring via config files instead of shell exports" under "LiteLLM" below.
...
| `LITELLM_MASTER_KEY`                             | required                                                         | litellm - admin credential for `/ui` and `/key/generate`; used by `make init` itself (`services/init/init_litellm_key.py`) to auto-provision your first team/key, and by the UI for any additional key afterward; real Anthropic/OpenAI keys are managed through the UI, see "LiteLLM" below                                                                                                                                                                       |
| `LITELLM_DB_PASSWORD`                            | required                                                         | litellm, litellm-db - Postgres password for LiteLLM's own virtual-keys/budgets database                                                                                                                                                                                                                                                                                                                                                                              |
| `LITELLM_TEAM_ID`                                | auto-provisioned                                                 | not read by any service - `make init` (`services/init/init_litellm_key.py`) records the default Team it created here, so a re-run reuses it instead of creating a second team; see "Issue yourself a personal key" above                                                                                                                                                                                                                                            |
