...
The stream's own `MAXLEN` (`services/_common/src/queue.yml`, shared by `webhook`/`webhook-worker`) is what actually keeps memory there in practice - see `AGENTS.md` for the sizing math.
