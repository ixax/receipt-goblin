...
Queue between `webhook` (producer) and `webhook-worker` (consumer) - see `agent_docs/services/webhook.md`'s "Why a queue in front of ClickHouse".
...
The stream's own `MAXLEN` (`services/_common/src/queue.yml`, shared by `webhook`/`webhook-worker` since the webhook-worker-split refactor - see `plans/webhook-worker-split.md`) is what actually keeps memory there in practice - see `AGENTS.md` for the sizing math.
