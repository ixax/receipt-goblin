...
`docker-entrypoint.sh` is baked in as `ENTRYPOINT`; the role choice lives in `docker-compose.yml`'s per-service `environment: APP_ROLE: ...`, not a compose-level `command:` override - keeps one image/Dockerfile for five behaviors instead of five near-duplicate images.
...
- ClickHouse handles a few large batched inserts far better than many small per-request ones (merge/part amplification, connection overhead per insert).
  The old design had `webhook`'s request handler doing up to 4 separate `client.insert()` calls per `StandardLoggingPayload`, synchronously, inside the HTTP handler - fine at low volume, but with many developers routing concurrent Claude Code/Codex sessions through LiteLLM, that's many small inserts per second hitting ClickHouse directly, with backpressure onto `webhook` (and from there, retries from LiteLLM) if it falls behind.
- `webhook` now only ever enqueues onto `redis`. `webhook-worker` is the only thing that talks to ClickHouse for `/api/v1/metrics` traffic - it's also the only thing that calls `clickhouse_ingest.build_event()` (per payload, on decode - pure, no I/O), in batches via `clickhouse_ingest.ingest_events_batch()`.
...
- Removing `build_event()` alone wasn't enough: `webhook-1`/`webhook-2` (each `cpus: 1.0`) still pegged at 65-96% CPU under the same load, even with `build_event()` gone.
  The culprit was `request.json()`/`json.dumps()` themselves - parsing and re-serializing a ~360KB-1.5MB `StandardLoggingPayload` twice per request (parse in, re-serialize out) is real CPU work.
  `queue_client.enqueue_raw(body: bytes)` is the fix: `server.py`'s handler reads `await request.body()` (raw bytes, not `.json()`) and, for the common single-payload case, `XADD`s those bytes onto `redis` completely unparsed - zero `json.loads`/`json.dumps` on webhook's request path.
  Only a bundled `log_format: json_array` body needs an actual parse to split it into one Redis entry per payload, falling back to `enqueue(payloads: list)` for that case.
...
- `server.py` - receives LiteLLM's webhook POSTs and calls `queue_client.enqueue_raw()` - no longer captures anything to disk itself (moved entirely to `loadtest-fixtures`, which reads FROM ClickHouse on demand, not live traffic).
  Never touches ClickHouse for `/api/v1/metrics` (still does for `/api/v1/session-git-branch` and `/api/v1/plan-proposal`).
  Those two routes verify the caller's `Authorization: Bearer <virtual key>` against LiteLLM's own `/key/info` (`config.LITELLM_MASTER_KEY`/`LITELLM_BASE_URL`) before accepting, rejecting with 401 otherwise.
...
- `clickhouse_ingest.py` - parses `StandardLoggingPayload` (incl. `Agent`/`Skill` tool_use blocks in `messages`).
  ...
  Also has `ingest_git_branch`/`ingest_plan_proposal`, called from `server.py`'s `/api/v1/session-git-branch`/`/api/v1/plan-proposal` routes, for `session_git_branch` (see `hooks/report_git_branch.py`) and `plan_proposals` (see `hooks/report_plan_proposal.py`) - both stay direct-to-ClickHouse, not queued (single low-volume insert per CLI session start / `ExitPlanMode` call).
...
- `config.yml` - queue-mechanics tunables loaded by `config.py` (the sizing rationale for each lives here, since it's the file you actually edit to tune them):
  - `stream_key`
  - `consumer_group`
  - `maxlen`
  - `batch_size`
...
- **Gotcha: `fastjson.dumps()` returns `bytes`, not `str`.** A caller that needs a `str` (e.g. for `Path.write_text()`, an f-string, or anything expecting stdlib's return type) must `.decode()` the result - stdlib `json.dumps()` doesn't need this, so a straight swap-in can silently break a caller expecting `str`.
- **Not fully applied yet - this is the most volatile part of this file, tracked here rather than in `AGENTS.md` so updates don't touch the cached prefix:** `worker.py`, `queue_client.py`, `server.py`, `clickhouse_ingest.py`, and `reparse.py` still have leftover raw `import json` calls alongside their `fastjson` usage - don't add a new one, but fixing the existing ones isn't required as a side effect of unrelated work.
  Applies to any new Python service too (e.g. `services/mcp-dev`, which currently imports neither, and `services/mcp-stats`, which uses stdlib `json`) - import `fastjson` from the start rather than reaching for stdlib `json`.
- **Same status for the `LOG_LEVEL`/stdlib-`logging` rule:** `services/webhook/src/{worker,migrate,loadtest,reparse,server}.py` still hardcode `INFO`, and `services/mcp-dev`/`services/mcp-stats`/`services/backup`/`services/init` don't use `logging` at all yet - all pre-existing gaps against the rule in `AGENTS.md`, not yet fixed.
