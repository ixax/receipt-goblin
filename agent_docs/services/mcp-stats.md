...
One tool, `me(session_id)`, reports:

- cost/tokens for the given session (must be the live `CLAUDE_CODE_SESSION_ID` env var)
- a global last-30-days rollup
- that session's 5 most expensive operations (`agent_usage` rows, `LEFT JOIN agent_events` by `litellm_call_id`)
