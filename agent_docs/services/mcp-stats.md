...
`_operation_label` picks a human-readable label per row: command/skill/agent/mcp-tool name if the call itself was one of those, else falls back to `agent_events.calculated_type` (spawned a subagent, ran a tool, answered plainly, etc) - a bare `llm:<model>` without that join is nearly always `claude-sonnet-5` repeated with no useful signal.
