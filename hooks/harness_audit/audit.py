...
def agent_yaml_description_words(text: str) -> int:
    """Same word-count budget as description_words(), but for an
    .agents/agents/*.yaml source: no frontmatter fence, description: is a
...
def check_description_word_budget(rel: str, kind: str, text: str) -> list:
    if kind not in ("skill", "agent", "agent_yaml"):
        return []
    dw = agent_yaml_description_words(text) if kind == "agent_yaml" else description_words(text)
...
def check_one_sentence_per_line(rel: str, kind: str, text: str) -> list:
    if kind not in ("agent", "skill", "rule", "root_md", "nested_md", "deep_dive", "other_md", "agent_yaml"):
        return []
...
def rule_candidate_lines(kind: str, text: str) -> list:
    ...
    kind "agent" (.claude/agents/*.md) is compiled from kind "agent_yaml"
    (.agents/agents/*.yaml) by scripts/compile_agents.py, so it legitimately
    duplicates its source line-for-line - excluded here the same way the
    symlinked .claude/skills copy is excluded via realpath dedup in
    collect(), so only the .agents/agents/*.yaml source participates in
    cross-agent duplicate detection."""
    if kind in ("other_md", "agent"):
        return []
    return [
    ...
    for dirpath, dirnames, filenames in os.walk(ROOT):
        ...
        for fn in filenames:
            ...
            if fn in ("CLAUDE.md", "AGENTS.md"):
                ...
                kind = "agent"
            elif fn.endswith(".yaml") and rel.startswith(".agents/agents/"):
                kind = "agent_yaml"
            elif fn.endswith(".md") and rel.startswith("agent_docs/"):
                # Hand-authored deep-dive docs (architecture.md, incidents.md) -
                # no token budget (not always-loaded), but still checked for
                # dead references and duplicate rules.
                kind = "deep_dive"
            elif fn.endswith(".md"):
